...
**Corrected the Explore/Haiku claim (item #8 in the April entry).** As of Claude Code v2.1.198 the built-in Explore agent inherits the session model rather than always running on Haiku; on a Sonnet session, recon runs on Sonnet. The `epc.md` text was updated, and this repo now ships an `Explore` override pinned to `model: sonnet` (`agents/explore.md`, read-only Read/Grep/Glob) so recon runs on Sonnet 5 rather than the inherited Opus 5 session model — see model routing below. Doc: [Built-in subagents](https://code.claude.com/docs/en/sub-agents#built-in-subagents)

**Model routing for Pro's shared usage pool.** All four specialists move `model: inherit` → `model: sonnet` (Sonnet 5), and the bundled `Explore` override is pinned to Sonnet 5 too. The orchestrator (`epc.md`) stays `model: inherit`, so it tracks your session model (Opus 5 when you're driving it) where routing judgment pays off most. Rationale: on Pro, Opus and Sonnet draw from a single usage pool, so a fan-out running on the inherited Opus 5 would compete with your interactive session and hit the 5-hour/weekly wall fast; Sonnet 5 is near-Opus and covers frontend JS/React/test/git comfortably. Escalate a single specialist to `model: opus` + `effort: xhigh` only for hard architecture. Docs: [Model configuration](https://code.claude.com/docs/en/model-config) · [Subagent model field](https://code.claude.com/docs/en/sub-agents#supported-frontmatter-fields)
...
**Kept on purpose.** Every specialist keeps `effort: high` — the ceiling on Sonnet 5 (`xhigh`/`max` require pinning an agent to Opus). `git-wizard` keeps `effort: high` and `memory: user` and takes no gate (it drives risk/confidence scoring and produces no app code to typecheck); only its model moved to Sonnet 5 with the others. Note on the April regression fixes below: they described Sonnet 4.6 / Opus 4.6, now legacy. `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1` has no effect on Sonnet 5 or Opus 4.7+ ([those always use adaptive reasoning](https://code.claude.com/docs/en/model-config)), so on your current models that workaround no longer applies — the April entry is kept as historical record.
