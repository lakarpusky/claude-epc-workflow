...
Multi-agent system for frontend development. One orchestrator command routes tasks to five specialized agents. Agents read skills (knowledge modules) before implementing.

## What This Is

One orchestrator (`/epc`) + five agents + 19 skills for frontend development.

**Orchestrator:**
- `epc` - Routes tasks to the right specialist, allocates token budgets, manages handoffs between agents, enforces quality gates. EPC is a command — not an agent. It doesn't write code.
...
/epc standard add JWT auth             # Standard, ~200 tokens
/epc architect redesign state          # Complex, ~400 tokens
/epc emergency revert broken deploy    # Production fix, ~50 tokens
/epc unbounded full security audit     # Deep dive, no limit
...
- **Standard:** ~200 tokens - Feature implementation (Explore → Plan → Code)
- **Architect:** ~400 tokens - Complex architecture, multi-agent coordination
- **Emergency:** ~50 tokens - Production fixes, fastest path
- **Unbounded:** No limit - Deep dive, comprehensive analysis

**Reasoning-first budget split:**

Agents spend most of their budget *thinking*, not *talking*. Reasoning is where quality comes from — catching edge cases, evaluating tradeoffs, avoiding mistakes. Output is the compressed delivery.

| Mode | Reasoning | Output | Reasoning % | Why |
|------|-----------|--------|-------------|-----|
| Emergency | 10 | 40 | 20% | No time to think — revert and go |
| Quick | 60 | 40 | 60% | Pattern match, verify, deliver |
| Standard | 130 | 70 | 65% | Explore problem, plan, deliver concisely |
| Architect | 300 | 100 | 75% | Wrong architecture = weeks of rework |
...
**Quality Gates**  
EPC enforces error handling, null checks, loading states, error boundaries, accessibility, and measured impact on every agent's output before approval.

**Confidence Scoring**  
Every non-trivial decision scored [1-10]. Below 5 = stop and ask. EPC's confidence is capped by the weakest agent score.

**Reasoning-First**  
Agents spend 60-75% of their budget thinking deeply, then deliver concise results. Wrong decisions are expensive — thorough reasoning prevents them.

## What This Is NOT

* Not a framework or build system
* Not a GUI or dashboard
* EPC is not an agent — it's the orchestrator command that routes and validates
